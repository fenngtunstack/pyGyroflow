...
| **D-06 余下** 其余调用方换用逐点家族 | `zooming/fov_iterative.py` **已换**（`538c8af`）：`_undistort_points_simple` 整份删除，改调 `undistort_points_with_rolling_shutter`；工作副本改成全量克隆，逐帧镜头数据与逐帧 stab 数据随之到位。`synchronization/estimate_pose/almeida.py` **已换**（`034d033`）：`_CameraK` 现在持有 `ComputeParams` 与时间戳，`delta` 走镜头反演；`params` 可省（省则退回旧的针孔行为），manager 侧新增 `_build_sync_compute_params()` 喂给 `AutosyncProcess`，贯通 `PoseEstimator` → `estimate_rotation`。`estimate_pose/` 三个 OpenCV 估计器 **已换**（`3697a72`）：dispatch 层统一先 `undistort_points_for_optical_flow`，两帧各按自己的时间戳。**还剩两处**：`find_offset/rs_sync.py`（上游 `rs_sync.rs:120-121` 逐轨迹反畸变，另有 `FindOffsetsRssync::new` 的读出时长逻辑——global_shutter 强制 0.01 ms、默认值用 scaled_fps——移植没有）、`find_offset/visual_features.py`（**根本不是上游算法的移植**：上游 `visual_features.rs::find_offsets` 是对匹配点对逐 offset 候选做 `undistort_points_with_rolling_shutter(pts, ts − offs, …)` 后的平方距离最小化（1 ms 粗搜 + 0.01 ms 细搜 + 90% 接受窗），移植却是一个 1-D 角速度互相关（对应上游 offset method 0 的 `essential_matrix::find_offsets` 也不存在）。上游 offset 编号：0=essential_matrix、1=visual_features、2=rs_sync）。**另**：method 2 的 ARRSAC 移植是独立缺口（见上「新发现的缺陷」首条） |
