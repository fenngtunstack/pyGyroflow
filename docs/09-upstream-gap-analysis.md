...
| # | 等级 | 项 | 证据 |
|---|---|---|---|
| A-01 | R | ~~**25 种格式只支持 4 种**~~ **分解为逐格式台账（6/25 已完成）**。已完成：GoPro、DJI、Sony（含 A-06 深挖）、Insta360、GCSV（`c69ec94`）、GyroflowProtobuf（`2cd49a2`）。**剩余 19 个的诚实定性**：每个都是 300–1000 行的独立解析器移植，且各需独立判定验证材料——(a) 规范公开、可合成验证：CAMM（Google 公开 spec）、WitMotion、Runcam、ArduPilot（数据 Flash 日志格式公开）、EspLog、PhoneApps（移动端 log 布局在 vendor 内）；(b) 文件格式存在但生态稀少、真实样本罕见：Vuze、KanDao、QoocamEgo、Cooke、SenseFlow、Freefly、Zcam、Xtra、BlackmagicBraw（BRAW 元数据在容器深处，需官方 SDK 对照）、RedR3d（专有 RED 专有格式，R3D 元数据布局逆向而来，无对照样本不可验证）；(c) 大体量：BlackBox（Betaflight CLI 日志，解析器本体 ~900 行）、Canon（`canon.rs` 含 init_lens_profile + get_time_offset，规模对标 Sony A-06）。**结论**：该行不是单一任务而是 19 个独立任务；(a) 组六个可无样本推进，(b)(c) 组受真实素材阻塞——与 A-02 证明的"合成验证可行"路线仅在规范完整时可用。逐格式开工前需各自立项 | [实测] |
| A-02 | R | ~~**GCSV/Protobuf 输入缺失**（`gyroflow.proto` + `gcsv.rs`，1307 行）~~ **已全部关闭**。GCSV 半：`c69ec94`（头块/缩放/方向编码/按列宽可选通道，12 项测试）。Protobuf 半：`2cd49a2`——`telemetry/gyroflow_proto.py` 移植 vendor `binary.rs`（1122 行）+ proto schema：通用 wire 解码、曝光优先级链、逐帧读出行权、相机时钟不重置、IBIS 符号翻转、quats_rotation 共轭、imu_rotation 后置应用、EIS 量化四元数走 mod.rs 重排怪癖、brand=Sony 接通深挖通道、mesh_warp→`get_mesh_correction_from`（sony.py 拆分出 dict 层核心）。18 项测试用**手工编码的 wire format**（编码器/解码器独立实现，非循环） | [实测] |
...
| A-04 | R | ~~**整个文件读进内存**~~ **已修**（`5f80436`）。两半都补齐：① 检测窗口按文件大小分级（`lib.rs:73-84` 原表：>100GB→500MB、>60GB→220MB、>30GB→180MB、>5GB→50MB、否则 5MB），此前固定 5MB；② 解析路径弃 `f.read()` 整文件拷贝，改 `mmap` 视图——上游把 seekable stream 交给 `parse`（按样本表 seek 读，从不整文件驻留），`mmap` 对四个解析器写入时依赖的契约（切片/`find`/`len`/整数索引/负切片）完全等价，驻留集降为仅触碰页、内核可回收，内存形态与上游一致，4K 长片不再 OOM。稀疏文件（`truncate`）让 6/31/61/101 GiB 档真实可测，16 项新测试（`tests/test_windowed_read.py`）。GCSV sidecar 仍整读——上游对 sidecar 同样整读（`std::fs::read`），非偏差 | [实测] |
| A-05 | R | ~~**FileMetadata 19 字段中 9 个声明但从不写入**~~ **5 个已接通**（`38edd7f`，按 `gyro_source/mod.rs:137-501` 填充逻辑）：`image_orientations`（GoPro IORI——保真复刻上游 `from_vector(Vector4::new(x,y,z,w))` 重排怪癖：载荷 (a,b,c,d) 存为 (b,c,d,a)，无 -x 翻转，zip 到 quats 时间戳）；`gravity_vectors`（GoPro GRAV：i16/SCAL 缺省 **32767**、全零流丢弃、与 quats 1:1 时按 IORI 旋转）；`digital_zoom`（DZST/DZMX：1+(DZST/100)·(DZMX−1)，DZMX 缺省 1.4，last-nonzero-wins）；`frame_readout_direction`（mod.rs:398-405 符号+±10000 哨兵，四解析器统一接线，Sony 0xe40e 带符号）；`per_frame_time_offsets` 的 **Insta360 半**（Exposure 记录 id 4/12 + TimeMap id 128，含上游 0.9ms 神秘项）。`lens_positions` 审计本已过时（Sony 0x8005 早前已写入）。**剩余四项归域**：`lens_params`（需 tag_map 架构 + RED/Canon 级 Imager/Lens 结构组 → A-01 族）、`camera_stab_data`/`mesh_correction`/Sony 的 `per_frame_time_offsets`（sony.rs `stab_collect`/`get_mesh_correction`/`get_time_offset` → A-06）。消费端已闭环：`imu_integration/converter.py` 早就接受 image_orientations、`frame_transform.py:360` 消费 digital_zoom、`project.py` 序列化往返两 map。18 项新测试 | [实测] |
| A-06 | R | ~~**Sony 专项全缺**~~ **已移植**（`be09273`）：`gyro_source/sony.py` 完整移植 gyroflow 侧 `sony.rs` 四函数——`init_lens_profile`（0xe421 → 11×6 SVD 拟合 + Newton 逆 → 档案，含坏焦距回退路径）、`get_time_offset`（0xe40c/e40d/e437/e435/e436，含 RX0M2 +1.5ms、f64::round 语义）、`stab_collect`+`stab_calc_splines`（IBIS 0xe40f/e450、OIS 0xe416 → ISTemp → 逐帧 CatmullRom `CameraStabData`）、`get_mesh_correction`（0xe42f+0xe423 → 前向+逆网格缓冲；逆用 scipy NelderMead 对 argmin 同族配置，**非逐位一致**，已标注）。接线进 `_parse_sony` 的深挖通道（mod.rs:415-504 顺序）。**真机 RX100M7 验证**：time offsets 345、OIS 样条 345 帧、真实 LensDistortion 系数建出档案（fx≈2823px, model=sony）、mesh 345 帧 9×9 自洽。关键实证：Imager 四 tag 实际编码是 u16×2 / u32→float（非 u32×2 / IEEE f32），由真机探针钉死；着色器侧 `distortion_models/sony.py` 至此有了系数来源。消费端（undistort 里的 stab/mesh 应用）为独立后续工作 | [实测] |
...
| A-09 | D | ~~**滤波边界行为不同**~~ **终判：有意保留的移植偏差**。上游零状态因果前后向；我们用 `sosfiltfilt`（奇数对称填充）与 `medfilt`（居中窗）。序列两端系统性不同——但 filtfilt 的奇对称填充是 scipy 信号处理栈的标准形态， gold 数据（Rust 移植版）比对覆盖积分器而非滤波前端；换成上游的因果链需重写滤波层且与 Python 生态惯例相悖。影响限于每端各一个滤波窗口长度的过渡段，记录于此 | [实测] |
| A-10 | D | ~~**VQF 磁力计输入不同**~~ **终判：当前管线下行为等价**。上游 `mod.rs:126` 硬编码 `let m = [0,0,0]`；我们在 `TimeIMU.magn` 有值时传真实磁力计——但**所有现有解析器均不填 magn**（逐一核对过），故传零与传 None 在当前代码路径下不可区分。该差异仅在"未来某格式提供磁力计"时激活，届时它是超集而非缺陷；已在本行钉住激活条件 | [实测] |
...
| B-08 | R | ~~**接受条件不同**~~ **复核为已实现**（`5f50c70` 审计）：`autosync.py:469` 已是上游条件 `|delay−initial| ≥ radius·0.9 → 拒绝`（raw delay 上、读出项之前）；"平坦地形"对比守卫仍在，但它是**额外的**拒绝条件而非替代，已归 B-28 族记录 | [实测] |
...
| B-12 | R | ~~**`per_frame_time_offsets` 全链路无消费者**~~ **复核为已实现**：`frame_transform.py:524-525` 与 `:691-693` 两处均按 `timestamp_ms + per_frame_time_offsets[frame]` 应用（frame_transform.rs:216 的原义）；A-05（`38edd7f`）之后生产端也已接通（Insta360/Sony/proto），闭环 | [实测] |
...
| B-15 | R | **rank<13 质量门缺失**。上游 `render_queue.rs:1485-1504` 按 `sync_data.rank` 跳过低运动段的偏移；我们完全不用 rank，改用 `_valid_sync_points`（>40ms 偏差剔除）与 `_drift_significant`（≥3 点、斜率>15ms、残差<10ms） —— **经复核为有意保留的移植偏差**：两者的角色相同（低运动窗的偏移不可信，宁可不用）；port 的门限按毫秒语义而非 rank 分位数，且 rank 曲线我们已在 `OptimSync.run` 产出（`_rank` 返回值，仅未消费）。B-28 一并记录该族自加启发式 | [报告] |
| B-16 | R | **`SyncParams` 结构缺失**：`custom_sync_pattern`/`auto_sync_points`/`every_nth_frame`/`calc_initial_fast`/`initial_offset_inv` 全无。且 `manager.py:673-674` 在 OptimSync 给 <2 点时直接放弃，上游 `render_queue.rs:1448-1462` 会退化成均匀分布 —— calc_initial_fast/initial_offset_inv 已补（`ed114ad`）；OptimSync 空点回退均匀分布与兼容设置 sync_settings 叠加已补（`126f7db`）；`custom_sync_pattern`/`auto_sync_points`/max_sync_points 旋钮已补（`5f50c70`：render_queue.rs:1443-1459 三旋钮 + resolve_syncpoint_pattern:1690-1727 完整移植，含帧数×fps/"ms"/"s"字面量与 gap-对分展开）—— **B-16 关闭** | [报告] |
...
| B-18 | D | 流式 API 缺失：无 `feed_frame`、无取消、无增量估计、无 OF 缓存生命周期。批处理 `run()` 功能上够用，但长视频无进度/无中断 —— **部分收窄**：`feed_frame` 已在（PoseEstimator），进度回调已贯通渲染（`6a81788`）与逐点精化，OF 缓存生命周期即 `cache_optical_flow`/`cleanup`（`bf6e5dd`）；缺的是估计器侧的中断标志与逐帧增量估计 —— **终判：有界后续工作**。两者都是估计器内部的控制流改造：取消标志需把 PoseEstimator/检测器的逐帧循环接一个 `AtomicBool` 等价物；增量估计需把批内 OF 缓存改为跨调用驻留。无算法风险、无素材依赖，但属于性能/交互层而非正确性，按质量门槛逐项单独立项 | [报告] |
| B-19 | D | `guess_imu_orientation` 过程不同：我们用 6 个三帧窗口 + `_compute_cost`（`manager.py:852-964`）；上游 `rs_sync.rs:190-222` 是 48 个朝向的 `pre_sync` 代价求和 —— 48 个朝向的枚举已在（`_POSSIBLE_ORIENTATIONS` 恰 48 项）；差异仅在窗口选取（6 个三帧窗口 vs 全程）与代价函数（`_compute_cost` vs crate `pre_sync`）。B-04 落地后代价侧可换 crate 的 pre_sync —— **终判：有界后续工作**。`rs_sync_problem.py` 已含 pre_sync 的逐位移植（B-04），把 `_compute_cost` 换成它的调用点是一处接线；6 窗口 vs 全程的采样差异保留为偏差（全程 48 次代价在串行 Python 下过重）。见 B-17 同族性能权衡 | [报告] |
...
| B-24 | R | ~~**OptimSync 缺低运动分支**~~ **复核为已实现**：`optimsync.py` 已有 `low_motion = max(mf) < _LOW_MOTION_MF_MAX(=50.0)` 分支切 `(lf+mf)/(1+nlfunc(hf,450)·0.003)`，常数与上游逐一对齐；`tests/test_optimsync.py::TestLowMotionBranch` 覆盖慢速平移场景。行描述过时 | [实测] |
| B-25 | D | ~~OptimSync 返回**已裁**的 rank~~ **已修**（`5f50c70`）：`run` 改回裁剪前快照 `rank_unclipped`（optimsync.rs:151 `rank_clone` 语义），选中点仍用裁剪后数组；测试钉住 trim 范围外能量在返回值中保留 | [报告] |
...
| # | 等级 | 项 | 证据 |
|---|---|---|---|
| C-01 | R | **输出编解码矩阵**。上游 H.264/H.265/AV1/ProRes(6 profile)/DNxHD(8)/CineForm/**EXR 序列**/**PNG 序列**；我们 libx264/libx265/prores_ks(坏)/libaom-av1，另加两个上游没有的（vp9/mpeg4）。**无序列输出**——输入侧做了序列，输出侧仍只有视频 —— **序列输出已实现**（PNG/EXR：`_SEQUENCE_CODECS` + printf 模式校验 + image2 容器 + 音频跳过 + CLI 模式生成，`tests/test_rendering.py` 覆盖）；ProRes 编码已在 G-01 修复。DNxHD 已补（`6445638`：yuv422p + 默认 36 Mb/s，实测编码器要求显式码率且只认 profile 分辨率档）；AV1 修复为真实可用（本机构建缺 libaom-av1，运行时回退 libsvtav1 并实测编码，顺带修探测函数吞 NameError 的 bug）。**CineForm 终判**：本构建 `cfhd` 仅解码可用、`avcodec_open2` 打开即 EINVAL——FFmpeg 主线无 CineForm 编码器，此环境内不可实现。ProRes 6 profile / DNxHD 8 档的 profile 级细项保留为后续 | [实测] |
...
| C-03 | R | ~~**底层缺像素格式类型**~~ **已修**（`0536f87`）：`stabilization/pixel_formats.py` 移植 pixel_formats.rs 的 13 种 PixelFormatTypes（NV12 UV 交换、P010 实为 16-bit 怪癖、GBR 平面序、YUV444P16LE 回退）+ rgb_to_yuv Rec709 limited 映射 + planes_for_format 分派表，`tests/test_pixel_format_types.py` 覆盖。C-02 的根因已除，逐平面消费端仍受 PyAV 解码格式覆盖限制（见 C-02） | [实测] |
...
| C-08 | R | 无 GPU 解码/编码（上游 `ffmpeg_hw.rs` 412 行 + 各平台 interop）；无 GPU 解码重试阶梯、无像素格式回退 —— **终判：硬件门控**。移植本身可行（412 行逐行搬运），但 hwaccel 的每条路径（VAAPI/NVDEC/VideoToolBox/MediaCodec）与解码重试阶梯只有在对应硬件上才能验证；本机 GPU 是 lavapipe 软件渲染（见 pygyroflow/CLAUDE.md），真硬件验证不可得。与 GPU undistort（wgpu 路径已验证）不同，解码/编码 interop 无法用软件栈等价验证。有对应硬件时单独立项 | [报告] |
...
| C-12 | R | ~~**RenderQueue 是骨架**~~ **stmap job 已接通**（G-06，`9dc14ab`/`b96c4bd` 前后）：export 类 job 与 stmap job 均走真实导出器。**剩余终判：有界后续工作（GUI 批渲染层）**——并行渲染（rayon→线程池/进程池映射）、暂停/取消、队列持久化、preset 批量、when_done 回调，全部是编排层设施，不含算法成分；每个都可独立实现与测试，无素材/硬件依赖。上游对应代码 1740 行中的算法部分（渲染前 autosync 即 render_queue.rs:1430-1520 的同步编排）已随 B-16/B-25 移植进 `manager._refine_sync_points` | [报告] |
...
| D-04 | R | ~~**地平线锁顺序反了且应用两次**~~ **已修**（G-08）：registry 内的锁定调用移除，全树仅 `manager.py:1176` 一处 `.lock(`，在平滑之后按上游 `gyro_source/mod.rs:626-629` 的语义应用一次 | [实测] |
