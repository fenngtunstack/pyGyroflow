...
## 2026-09-15 官方工程逐帧对照（验证债务偿还）

**工具**: `tests/verify_against_official.py` —— 解析官方 .gyroflow 工程（JSON，含完整复现参数：镜头标定、8项平滑参数、3点同步偏移、输出尺寸），pyGyroFlow 按工程参数重渲同素材，逐帧 PSNR/MAD 对比官方成片；`--sync-only` 只做同步级对照。

**同步级发现**（已完成）:
- 官方 Hero6 工程 offsets = **3 点同步**（5.61s:49.13 / 16.95s:49.32 / 28.29s:50.16 ms），34s 内漂移 +1.04ms——上游多点同步的真实样本
- 我们 autosync 全局 41.00ms、多点守卫正确拒绝 1ms 级微漂移（<15ms 门限，信号小于 gpmd 量化噪声，设计符合）
- **41 vs 49ms 的 8ms 差即 gpmd 量化误差带的具体体现**：两边管线的均匀网格 anchor 不同（stts 时长/读数 vs 官方实现），各自管线内各自最优——格式层天花板，非缺陷
- 官方工程 smoothing_params 8 项中 7 项与我们 DefaultAlgo 参数名对齐；`second_pass` 虽无参数名暴露但行为默认开启（与上游一致，嫌疑排除）

**像素级逐帧结果**（官方参数重渲 1019 帧 vs 官方成片，PSNR）:
- 官方 offsets（49ms）版：中位 18.02 dB；我们最优（41ms）版：中位 18.21 dB——**同步失配非主因**
...
## 2026-09-16 EWA 插值实现（P4 清单第四项）

**背景**: 索引 3-6（RobidouxSharp/Robidoux/Mitchell/Catmull-Rom）此前一律退回 Lanczos4。EWA（Elliptical Weighted Average）与固定核滤波的本质区别：**缩小采样时核要随局部雅可比拉伸**，否则就是在点采样、必然走样——稳定化里 FOV 放大（adaptive zoom 拉远）和镜头校正的画面边缘压缩都会造成缩小采样。

**上游实现**（`core/stabilization/cpu_undistort.rs`）:
- 索引→核半径：Bilinear `I=2`、Bicubic `I=4`、Lanczos4 `I=8`、**EWA `I∈{10,11,12,13}`**；`if I > 8` 走 EWA 分支
- 四个 EWA 其实是 Keys 三次滤波族（CubicBC）的四组 (B,C)：RobidouxSharp (0.2620145, 0.3689927)、Robidoux (0.3782157, 0.3108921)、Mitchell (1/3, 1/3)、Catmull-Rom (0, 0.5)；系数 `ewa_coeffs_p/q` 就是这组 (B,C) 展开的多项式
- **雅可比是有限差分算出来的**（对映射函数在 `eps=0.01` 处前向差分），不是解析求导——这点很关键，说明数值求导本就是参考实现的做法
- 每像素：`affine_bbox` 定采样框、`clamped_ellipse` 定椭圆二次型，框内 `k=bc2(sqrt(fx²a+fx·fy·b+fy²c))`，累加 `k·pixel / Σk`；**框外采样替代为背景色并保留其权重**（不是剔除后重新归一）

**实现**（`pygyroflow/stabilization/ewa.py`，新增 321 行）:
- 雅可比取自**已算好的 remap 图的中心差分**（`map_jacobian`），与上游的前向差分二阶等价且不额外付代价——映射图本来就要算。**从裁剪前的图取**：背景模式（边缘重复/镜像）会把区域压平，椭圆会退化
- 无效像素的占位坐标（`-1e5`）会让差分爆炸、把全帧采样框撑到半径上限，因此对无效像素及其一环邻域把雅可比中性化为单位阵（这些像素最终画背景色，雅可比只要有限即可）
- EWA 无法用 `cv2.remap` 表达（核逐像素变长），改为 numpy 逐 tap gather；每个 chunk 行分块处理限制峰值内存
- 循环范围是 `[-R, R+1]` 而非 `[-R, R]`：采样框是 `[floor(uv-b), ceil(uv+b)]`，小数部分会把上界顶出 `floor(uv)+R` 一格（这一步错了会让帧边缘一圈对不上——正是首版标量/向量对照测试抓出来的）
- 帧外 tap 用背景色并按原权重计入（复刻上游 `*bg` 语义，背景会向画面边缘渗入）；整数化用「+0.5 后截断」并 clamp——负瓣核在边缘会和背景色产生超range 的加权和，不 clamp 会 uint8 回绕成黑点

**验证**（472 passed, 5 skipped；新增 `tests/test_ewa.py` 57 项 + 改写 3 项插值测试）:
- **标量参照对照**：把上游 EWA 分支逐行转写成 Python 纯循环（`affine_bbox`/`clamped_ellipse`/`bc2` 全部独立实现），与向量化实现在 12 种 (滤波器 × 雅可比) 组合上逐像素比对通过。这是没有上游二进制时最强的等价性证据
- 不变量：**常数图在任意雅可比下精确还原**（权重归一化，四滤波器 × 三种雅可比）、帧外区域取背景色、无效像素取背景色、边缘背景渗入、输出不越界不回绕
- 几何：单位雅可比→单位圆与 ±2 框、2 倍压缩→x 方向框宽到 4、旋转雅可比在两轴都有足够覆盖、退化雅可比被 clamp 不爆炸
- 核：四个滤波器 (B,C) 与上游逐值一致、支撑界在 |x|=2、只有 Catmull-Rom 在 |x|=1 为零（B=0 才是插值核）
- 端到端：`--interpolation ewa-mitchell` 走通 manager→render 全链（12 帧合成片，输出帧数一致）

**性能（重要，已实测并写进代码告警）**:
- 1080p 单帧：Lanczos4 ~2.1 s（含建图），EWA **16-23 s**；4K：Lanczos4 ~6.5 s，EWA **49-154 s**
- 量级来源：EWA 是逐 tap 的 numpy gather（半径 3 时 8×8=64 tap × 200 万像素），且向量化实现用**全帧最大半径**统一循环（上游每像素只跑自己的框，平均 tap 更少）
- **结论：CPU 路径不是 EWA 该待的地方**——上游是在 fragment shader 里做的。这里保留实现（正确、可验证、可做 GPU 对照），CLI 的 `--interpolation` 暴露这四个值但帮助文本写明代价，`cpu_undistort` 首次走到 EWA 时打一次告警。要真正用起来得走 GPU 路径（后续项）

**已知限制**:
- 采样半径上限 `MAX_RADIUS=6`（13×13），上游无上限；超大缩小采样时被截断（仅超限时）
- 每 tap 的边界处理与整块 gather 让向量化版本比"逐像素只跑自己的框"多算约 1.4-2 倍
- 未做 float32 累加（保持 float64 以维持常数还原的严格相等与标量对照的紧容差）——若将来要提速，这是第一刀
