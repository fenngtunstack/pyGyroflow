...
PyGyroFlow就是为解决这个问题做的——把Gyroflow v1.6.3的核心算法完整移植到Python。不是包装一层Rust FFI，是真的用Python重写。最终的数据：108个Python源文件，18215行代码，180个测试用例全部通过，与Rust原版的数值误差在机器精度级别（max_err < 1e-14）。
...
18215行Python代码去替代69000行Rust代码，代码量少了将近4倍。这不是因为Python更"简洁"——Rust版有很多GPU渲染代码、跨平台UI代码、视频编解码代码，这些PyGyroFlow要么用了现成库（wgpu-py、PySide6、OpenCV），要么暂时不需要。
